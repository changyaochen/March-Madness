...
earliest_season       = 2003 # including this year
single_season         = 2017
...
if train_csv_file:
#  pass
  train = pd.read_csv('train_v4_2017_to_2017.csv')
else:
  ...
  if Train_Tourney:
    df = Tourney_detail.loc[Tourney_detail.Season >= earliest_season, :].copy()
    df['Type'] = 'T'  # Tourny
    train = train.append(
...
#    # =====================
  ...
  # remove useless columns_
  col_to_del = ['index', 'level_0', 'level_1', 'level_2', 'level_3']
  for x in col_to_del:
    if x in train.columns:
      del train[x]
  
  train.to_csv('train_v4_temp_'+str(earliest_season)+'_to_2017.csv', index = False)
...
#train = train[train['Season'] == single_season]
...
print('\nStart training...')
count_total = 1
RF_para = np.linspace(4, 12, count_total)
NN_para = np.logspace(-4, 4, count_total)
GLM_para = np.logspace(-4, 0, count_total)
GBRT_para = np.linspace(4, 12, count_total)
...
#  features = fe.copy()
  ...
  if GLM:
    print('\nLogistic regression:')
    start = time.time()
    glm = LogisticRegression(C = 1e-3)
    glm_train_sub, glm_xv_sub = train_test_split(train, test_size = 0.2)
    glm_target = train['result']
    glm.fit(glm_train_sub[features], glm_train_sub['result'])
    
    print('Training Accurancy : {:<10}'
          .format(glm.score(glm_train_sub[features], glm_train_sub['result'])))
    print('x-validation Accurancy: {:<10}'
          .format(glm.score(glm_xv_sub[features], glm_xv_sub['result'])))
    GLM_scores.append(glm.score(glm_xv_sub[features], glm_xv_sub['result']))
    print('Time spent for GLM: {:6.3f}s'.format(time.time() - start))
    print('The logloss is: {}'.format(logloss(glm_xv_sub['result'], 
          glm.predict_proba(glm_xv_sub[features])[:, 1])))
    
  if GBRT:
    print('\nGradient Boost RT:')
    start = time.time()
    gbrt_clf = GradientBoostingClassifier(n_estimators=1000, learning_rate=1.0,
                        max_depth=5, random_state=0)
    # split the training set, for x-validation
    train_sub, xv_sub = train_test_split(train, test_size = 0.2)
    target = train['result']
    gbrt_clf.fit(train_sub[features], train_sub['result'])
    print('Training Accurancy : {:<10}'
          .format(gbrt_clf.score(train_sub[features], train_sub['result'])))
    print('x-validation Accurancy: {:<10}'
          .format(clf.score(xv_sub[features], xv_sub['result'])))
...
if predict:
  if prediction_csv_file:
    matches = pd.read_csv('matches_preditions_v4_2017.csv')
  else:
    matches = pd.read_csv('./input/SampleSubmission_2017.csv')
    ...
    matches.to_csv('matches_preditions_v4_temp.csv', index = False)
  ...
  if GLM:
    GLM_pred = glm.predict_proba(matches[features]) 
  if GBRT:
    GBRT_pred = gbrt_clf.predict_proba(matches[features]) 
   
  submission_NN =  pd.read_csv('./input/SampleSubmission_2017.csv')
  submission_NN['pred'] = NN_pred[:, 1]
  submission_NN.to_csv('submission_NN.csv', index = False)
  
  submission_RF =  pd.read_csv('./input/SampleSubmission_2017.csv')
  submission_RF['pred'] = RF_pred[:, 1]
  submission_RF.to_csv('submission_RF.csv', index = False)
  
  submission_GLM =  pd.read_csv('./input/SampleSubmission_2017.csv')
  submission_GLM['pred'] = RF_pred[:, 1]
  submission_GLM.to_csv('submission_GLM.csv', index = False)
  
  # calculate the logloss with all 0.5 probability benchmark, should be 0.693147
  benchmark = submission_RF.copy()
  benchmark['pred'] = 0.5
  print('The logloss of RF against benchmark is: {}'.format(logloss(benchmark['pred'], 
          submission_RF['pred'])))
  print('The logloss of NN against benchmark is: {}'.format(logloss(benchmark['pred'], 
          submission_NN['pred'])))
