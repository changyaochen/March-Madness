...
#earliest_season       = 2013 # including this year
single_season         = 2017

train_csv_file        = True
Train_Regular_sesaon  = False
Train_Tourney         = True
...
#    # =====================
  ...
  # remove useless columns_to_del = ['index', 'level_0', 'level_1', 'level_2', 'level_3']
  for x in col_to_del:
    if x in train.columns:
      del train[x]
  
  train.to_csv('train_Tourney_temp_'+str(earliest_season)+'_to_2017.csv', index = False)
...
if predict:
  if prediction_csv_file:
    matches = pd.read_csv('matches_preditions_2017.csv')
  else:
